...
class Trainier(nn.Module):
    def __init__(self, save_model_weights_path, save_val_results_path, fold, device, patch_size, lr, num_epochs): 
        ...
        if fold == 'fold1':
            ...
            checkpoint = torch.load(os.path.join('/workspace/DB2_proposed_model_fd_fold1_weights', 'model_195.pth'))# Best loss index: 130 Best loss:  0.0003605265722977823
            LensFlareMask_extractor.load_state_dict(checkpoint['model_state_dict'])
            print('flare CAM fold1 checkpoint loading complete...')
        elif fold == 'fold2':
            LensFlareMask_extractor = CAM()
            checkpoint = torch.load(os.path.join('/workspace/DB2_proposed_model_fd_fold2_weights', 'model_143.pth'))#Best loss index: 130 Best loss:  0.0003605265722977823
            LensFlareMask_extractor.load_state_dict(checkpoint['model_state_dict'])
            print('flare CAM fold2 checkpoint loading complete...')
    ...
    def _valid_epoch(self, epoch, val_loader):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(tqdm(val_loader)):
                ...
                val_highres_cam = torch.sum(torch.stack([F.interpolate(o, size, mode='bilinear', align_corners=False) for o in val_fmd_outputs]), 0)
